...
const WishlistPage = () => {
  ...
  const handleRemoveItem = async(productId) => {
    try {
      const response=await axios.delete('http://localhost:3000/course/wishlist/remove',{params:{userId:user._id,courseId:productId}});
  ...
  const handleAddToCart = async (courseId, price) => {
    try {
      ...
      const response = await axios.post('http://localhost:3000/course/cart', { 
        courseId, 
        userId: user._id 
      });
